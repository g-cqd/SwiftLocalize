...
public struct ConfigurationLoader: Sendable {
    ...
    private func validateProvider(_ provider: ProviderConfiguration) -> [ConfigurationIssue] {
        ...
        case .openai, .anthropic, .gemini, .deepl:
        ...
        case .cliGemini, .cliCopilot, .cliCodex:
        ...
        case .appleTranslation, .foundationModels:
...
public enum ConfigurationIssue: Sendable, Equatable {
    ...
    public var message: String {
        switch self {
        case let .warning(msg), let .error(msg):
            return msg
        }
    }
...
extension Configuration {
    ...
    public func resolveAPIKey(for provider: ProviderName) -> String? {
        guard let providerConfig = providers.first(where: { $0.name == provider }),
              let apiKeyEnv = providerConfig.config?.apiKeyEnv else {
            return nil
        }
